...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function generateMandala() {
    ...
    let layers: number;
    let symmetry: number;
    const minLayers = 7, maxLayers = 14;
    const minSymmetry = 20, maxSymmetry = 50;
        ...
        switch (shapeType) {
          ...
          case 2: // Woven Ring (Blueprint Version)
            if (j === 0) { // This is a full-ring shape, so we only draw it once.
              ...
              wovenPath.linewidth = STROKE_WIDTH;
          ...
          case 3: // Geometric Lattice (Blueprint Version)
            ...
            line1.linewidth = STROKE_WIDTH;
            line2.stroke = strokeColor;
            line2.linewidth = STROKE_WIDTH;
            break;
          ...
          case 4: // Boundary Ring (Blueprint Version)
            if (j === 0) { // Only draw once per layer
              ...
              outerBoundary.linewidth = STROKE_WIDTH;
            }
            break;
          ...
          case 5: // Gradient Ring (Blueprint Version)
            if (j === 0) { // Only draw once per layer
              ...
              innerBoundary.linewidth = STROKE_WIDTH;
          ...
          case 7: // Scalloped Ring (Blueprint Version)
            if (j === 0) { // This is a full-ring shape, so we only draw it once.
              ...
              scallopPath.linewidth = STROKE_WIDTH;
          ...
          case 9: // Triangles (Blueprint Version)
            ...
            triangle.linewidth = STROKE_WIDTH;

            two.add(triangle);
            break;

          case 10: // Dashed Line Ring (Blueprint Version)
            if (j === 0) { // Only draw once per layer
          ...
          case 11: // Teardrop Petals (Blueprint Version)
            ...
            teardrop.linewidth = STROKE_WIDTH;
          ...
          case 12: // Leaf Petals (Blueprint Version)
          ...
          case 15: // Parametric Leaf (Blueprint Version)
            ...
            for (let k = 0; k <= l_steps; k++) {
              const t = (Math.PI / l_steps) * k;
              ...
              l_points.push(new Two.Anchor(x, y));
            }
            const leafPath_bp = new Two.Path(l_points, false, false);
            const vein_bp = new Two.Line(0, 0, l_length, 0);
            ...
            shape_bp.linewidth = STROKE_WIDTH;
            vein_bp.linewidth = STROKE_WIDTH; // Make the vein slightly thinner
